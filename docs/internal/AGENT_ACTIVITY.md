...
| 2026-08-01 | Claude Code | Mitigated Render free-tier cold starts: added stdlib-only `backend/keepalive.py`, documented the free UptimeRobot/cron-job.org `/health` ping setup in README + render.yaml, and clarified Render cron jobs need a paid instance type | `backend/keepalive.py`, `README.md`, `render.yaml`, `AGENTS.md`, `AGENT_ACTIVITY.md` |
| 2026-08-01 | Claude Code | Made the keep-alive fully free and in-repo: added `.github/workflows/keep-alive.yml` (GitHub Actions scheduled ping of `/health` every 5 min, unlimited minutes on the public repo, `HEALTH_URL` repo-variable override), and switched README/render.yaml guidance to lead with it (external monitor now a backup) | `.github/workflows/keep-alive.yml`, `README.md`, `render.yaml`, `AGENTS.md`, `AGENT_ACTIVITY.md` |
| 2026-08-01 | Claude Code | Dropped Render hosting, made Vercel the single-project deploy (SPA + FastAPI same origin). Mounted the whole API under `/api` via a gateway in `main.py`, switched DB engine to `NullPool` for Vercel serverless, prefixed every frontend API call with `/api`, deleted `render.yaml`, repointed keep-alive tooling at the Vercel `/api/health` URL, documented Vercel deploy + keep-warm steps in README. Backend routes verified via import; frontend `npm run build` passes | `backend/app/main.py`, `backend/app/core/database.py`, `frontend/src/api/client.ts`, `frontend/src/api/auth.ts`, `frontend/src/api/courses.ts`, `render.yaml` (deleted), `backend/keepalive.py`, `.github/workflows/keep-alive.yml`, `README.md`, `AGENTS.md`, `AGENT_ACTIVITY.md` |
