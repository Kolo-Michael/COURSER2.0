    ...
    >
      <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="courser-card p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-xl font-bold text-stone-900">Add a free course</h2>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <label className="md:col-span-2">
              <span className="text-sm font-semibold text-stone-700">Course title</span>
              <input
                value={title}
                onChange={(event) => setTitle(event.target.value)}
                required
                placeholder="Example: Product Design Foundations"
                className="mt-1 w-full rounded-lg border border-stone-200 px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/25"
              />
            </label>
            ...
            <label>
              <span className="text-sm font-semibold text-stone-700">Level</span>
              <select
                value={level}
                onChange={(event) => setLevel(event.target.value)}
                className="mt-1 w-full rounded-lg border border-stone-200 px-3 py-2.5 text-sm capitalize focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/25"
              >
            ...
            <label>
              <span className="text-sm font-semibold text-stone-700">Course cover picture</span>
              <input
                type="file"
                accept="image/*"
                onChange={(event) => {
            ...
            <label className="md:col-span-2">
              <span className="text-sm font-semibold text-stone-700">Course description</span>
              <textarea
                value={description}
          ...
          <div className="mt-8 border-t border-stone-200 pt-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-bold text-stone-900">Organized course outline</p>
                <p className="mt-1 text-sm text-stone-600">Build the structure learners will follow: modules first, lessons inside each module.</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-4">
              {modules.map((module, moduleIndex) => (
                <div key={moduleIndex} className="rounded-lg border border-stone-200 bg-stone-50 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-bold text-stone-900">Module {moduleIndex + 1}</p>
                    {modules.length > 1 ? (
                      <button
                        type="button"
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    <label>
                      <span className="text-sm font-semibold text-stone-700">Module title</span>
                      <input
                        value={module.title}
                    ...
                    <label>
                      <span className="text-sm font-semibold text-stone-700">Module description</span>
                      <input
                  ...
                  <div className="mt-4 space-y-3">
                    {module.lessons.map((lesson, lessonIndex) => (
                      <div key={lessonIndex} className="rounded-lg border border-stone-200 bg-white p-3">
                        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_120px_auto]">
                          <label>
                            <span className="text-sm font-semibold text-stone-700">Lesson {lessonIndex + 1}</span>
                            <input
                              value={lesson.title}
                          ...
                          <button
                            ...
                            className="self-end rounded-lg px-3 py-2.5 text-sm font-semibold text-stone-500 hover:bg-stone-100 disabled:cursor-not-allowed disabled:opacity-40"
                        ...
                        <textarea
                          ...
                          className="mt-3 w-full rounded-lg border border-stone-200 px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/25"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="mt-3 rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm font-semibold text-stone-700 hover:bg-stone-50"
                  >
                    <i className="fa-solid fa-plus mr-2" aria-hidden />
                    Add lesson
                  </button>
        ...
        <aside className="space-y-6">
              ...
              {environmentStyles.map((item) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-3 text-center text-xs font-semibold ${
                    environment === item.id ? 'border-primary bg-blue-50 text-primary' : 'border-stone-200 text-stone-700'
                  }`}
                >
              ...
              {mascots.map((item) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg border p-3 text-left ${
                    mascot === item.id ? 'border-primary bg-blue-50' : 'border-stone-200'
                  }`}
                >
                  <span className={`relative h-10 w-10 rounded-full ${item.accent}`}>
                    ...
                    <span className="absolute bottom-3 left-1/2 h-1 w-4 -transtone-x-1/2 rounded-full bg-white" />
                  </span>
                  <span>
                    <span className="block text-sm font-bold text-stone-900">{item.name}</span>
                    <span className="block text-xs text-stone-500">Answers learner questions</span>
                  </span>
                </button>
              ))}
            </div>
          ...
          <section className="courser-card overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-semibold uppercase text-stone-500">{environment}</span>
              </div>
              <h3 className="mt-3 font-bold text-stone-900">{title || 'Course title preview'}</h3>
              ...
              <div className="mt-4 rounded-lg bg-blue-50 p-3">
                ...
                <p className="mt-1 text-sm text-stone-700">Ask me anything about this lesson.</p>
              </div>
              <div className="mt-4 rounded-lg border border-stone-200 p-3">
                <p className="text-xs font-semibold uppercase text-stone-500">Outline</p>
          ...
          <label className="flex items-center gap-3 rounded-lg border border-stone-200 bg-white p-4 text-sm font-semibold text-stone-700 shadow-sm">
            <input
              type="checkbox"
              checked={published}
